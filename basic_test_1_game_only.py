...
#----------- code -----------
def setup_once():
    global player_name, num_games_chosen, player_info
    while(True):
...
def categoryselection():
    global categories, selected_category, category
...
def main_game():
    ...
    word_length = {}
    hidden_word = {}
    '''.format(word_length, hidden_word)) """
    window()


def window():
    global btm_lbl, root, guess_ent, lbl_1, draw
    #-Main Making of the window
    root = tk.Tk()
    root.title('Hangman')
    ...
    ent_btn = tk.Button(btm_frame, text='submit', command= lambda: strikes == submit())
    ent_btn.pack(anchor='n')

    #this will lift the window to the front on open and make it full screen
    root.lift()
